...
interface CheckoutProps { cart: CartItem[]; total: number; userProfile?: any; onComplete: () => void; onCancel: () => void; }

const Checkout: React.FC<CheckoutProps> = ({ cart, total, userProfile, onComplete, onCancel }) => {
  const [step, setStep] = useState<1 | 2 | 3>(1);
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState({ name: userProfile?.fullName || '', phone: userProfile?.phone || '', city: userProfile?.city || '', detailedAddress: '', momoId: '', });

  useEffect(() => {
    if (userProfile) setForm(prev => ({ ...prev, name: prev.name || userProfile.fullName || '', phone: prev.phone || userProfile.phone || '', city: prev.city || userProfile.city || '', }));
  }, [userProfile]);

  const handleFinish = async () => {
    setLoading(true);
    try {
      await addDoc(collection(db, 'orders'), { items: cart, totalAmount: total, status: OrderStatus.PENDING_VERIFICATION, momoTransactionId: form.momoId.toUpperCase(), createdAt: new Date().toISOString(), customerName: form.name, customerPhone: form.phone, deliveryAddress: `${form.city}, ${form.detailedAddress}`, userId: userProfile?.uid || 'guest' });
      setStep(3);
  ...
  if (step === 3) return (
    <div className="max-w-xl mx-auto py-24 px-4 text-center animate-in zoom-in duration-300">
      <div className="w-24 h-24 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-8 border-4 border-white shadow-xl"><CheckCircle2 className="text-green-500" size={48} /></div>
      ...
      <button onClick={onComplete} className="w-full bg-stone-900 text-white py-5 rounded-2xl font-bold shadow-xl shadow-stone-900/10 hover:bg-stone-800 transition-all">Return to Collection</button>
    </div>
  );

  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 lg:p-12">
      <button onClick={onCancel} className="flex items-center gap-2 text-stone-500 hover:text-stone-900 mb-10 font-bold text-sm uppercase tracking-widest transition-colors"><ChevronLeft size={20} /> Back</button>
      <div className="grid lg:grid-cols-5 gap-16">
          ...
          {step === 1 ? (
            <div className="space-y-6 animate-in slide-in-from-left duration-300">
              ...
              <textarea 
                ...
                placeholder="Detailed address..." value={form.detailedAddress} onChange={e => setForm({...form, detailedAddress: e.target.value})}
              />
              <button disabled={!form.name || !form.phone || !form.city || !form.detailedAddress} onClick={() => setStep(2)} className="w-full bg-stone-900 text-white py-5 rounded-2xl font-bold hover:bg-stone-800 disabled:bg-stone-200 transition-all">Continue to Payment</button>
            </div>
          ) : (
            <div className="space-y-8 animate-in slide-in-from-right duration-300">
              ...
              <input 
                autoFocus className="w-full p-5 border-2 border-stone-200 rounded-2xl outline-none focus:border-orange-600 transition-all text-xl font-mono font-bold text-black" 
                placeholder="ID87236592" value={form.momoId} onChange={e => setForm({...form, momoId: e.target.value})}
              />
              <div className="flex gap-4">
                <button onClick={() => setStep(1)} className="px-8 py-5 border-2 border-stone-200 rounded-2xl font-bold text-stone-600">Back</button>
                <button disabled={loading || !form.momoId} onClick={handleFinish} className="flex-grow bg-stone-900 text-white py-5 rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-stone-800 transition-all">{loading ? <Loader2 className="animate-spin" size={20} /> : 'Place Order'}</button>
              </div>
            </div>
          )}
        </div>
        <div className="lg:col-span-2">
          <div className="bg-stone-50 p-8 rounded-[2.5rem] border border-stone-200/60 sticky top-24">
            ...
            <div className="space-y-3 pt-8 border-t border-stone-200 text-2xl font-serif font-bold text-stone-900">
